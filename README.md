...
## Running emptyfield in Quarkus dev mode
...
```
(emptyfield should be included in the returned array)
...
[emptyfield-schema.json](./src/main/resources/schemas/emptyfield-schema.json ), run the following to create an
instance of the emptyfield workflow:
```shell script
curl -v -X POST -H "Content-Type: application/json" http://localhost:8080/emptyfield -d@input.json
```
